...
public class DirectionModule extends HudModule {
    ...
    @Override public void render(DrawContext ctx,MinecraftClient client){
        ...
        int x=config.x,y2=config.y,tw=client.textRenderer.getWidth(d);
        if(config.showBackground)ctx.fill(x-3,y2-2,x+tw+3,y2+10,0x90000000);
        ctx.drawText(client.textRenderer,d,x,y2,color,true);
    }
    @Override public int getWidth(MinecraftClient c){return 30;}
}